...
public class HibernateDao<T, PK extends Serializable> extends SimpleHibernateDao<T, PK> {
	/**
	 * 用于扩展的DAO子类使用的构造函数 通过子类的泛型定义取得对象类型Class. eg. public class UserDao extends HibernateDao<User, Long>{ }
	 */
	public HibernateDao() {
		super();
	}
	...
	/**
	 * 用于Service层直接使用HibernateDAO的构造函数. 在构造函数中定义对象类型Class. eg. HibernateDao<User, Long> userDao = new HibernateDao<User, Long>(sessionFactory, User.class);
	 */
	 ...
	 * 
	 * @param page
	 *            分页参数.支持pageSize、firstResult和orderBy、order、autoCount参数. 其中autoCount指定是否动态获取总结果数.
	 * @param criterions
	 *            数量可变的Criterion.
	 * @return 分页查询结果.附带结果列表及所有查询时的参数.
	 */
	...
	protected long countCriteriaResult(final Criteria c) {
		...
		try {
			orderEntries = (List) GenericReflectionUtils.getFieldValue(impl, "orderEntries");
			GenericReflectionUtils.setFieldValue(impl, "orderEntries", new ArrayList());
		} catch (Exception e) {
		...
		try {
			GenericReflectionUtils.setFieldValue(impl, "orderEntries", orderEntries);
		} catch (Exception e) {
			logger.error("不可能抛出的异常:{}", e.getMessage());
		}
