...
public class Facebook implements Serializable {
	...
	/**
	 * If only the access token is passed, then the Apache Http Client library is used for making http requests
	 * 
	...
	public User[] getUsers(String[] fbIds) throws FacebookException {
		...
		NameValuePair[] nameValuePairs = { new NameValuePair(Constants.PARAM_ACCESS_TOKEN, this.authAccessToken.getAccessToken()), new NameValuePair("ids", concatenatedFbIds) };
		User[] users = null;
	...
	public Page[] getPages(String[] fbIds) throws FacebookException {
		...
		Type type = new TypeToken<Map<String, Page>>() {
		}.getType();
	...
	/**
	 * Deprecated: Use {@link #link(List)} instead Posts a link on the user's/page's wall <br>
	 ...
	 * @param caption
	 *            The caption of the link, appears beneath the link name (optional)
	 * @param description
	 *            A description of the link, appears beneath the link caption (optional)
	 * @param message
	 *            The message from the user about this link(optional)
	 * @param icon
	 *            A URL to the link icon that Facebook displays in the news feed (optional)
	 * @param picture
	 *            A URL to the thumbnail image used in the link post (optional)
	 * @param privacy
	 * @throws FacebookException
	 */
	/*
	 * @Deprecated public void postLink(String link, String name, String caption, String description, String message, String icon, String picture, Value privacy) throws FacebookException {
	 * 
	 ...
	 * 
	 * if (name != null) { namesValues.add(new NameValuePair(Constants.NAME, name)); } if (caption != null) { namesValues.add(new NameValuePair(Constants.CAPTION, caption)); } if (description != null) { namesValues.add(new NameValuePair(Constants.DESCRIPTION, description)); } if (message != null) {
	 * namesValues.add(new NameValuePair(Constants.MESSAGE, message)); } if (icon != null) { namesValues.add(new NameValuePair(Constants.ICON, icon)); } if (picture != null) { namesValues.add(new NameValuePair(Constants.PICTURE, picture)); } if(privacy != null){ namesValues.add(new
	 * NameValuePair(Constants.PRIVACY,"{\"value\":\""+privacy.toString()+"\"}")); }
	 * 
	 * NameValuePair[] nameValuePairs = new NameValuePair[namesValues.size()]; namesValues.toArray(nameValuePairs); caller.postData(Constants.FACEBOOK_GRAPH_URL + Constants.POST_LINK.replaceFirst("PROFILE_ID", "me"), nameValuePairs); }
	 */

	/**
	 * Posts the given url on facebook <br>
	 * Internally calls {@link Facebook#postLink(String, String, String, String, String)} with other params null. If you want more control call that method
	 * 
	 * @param link
	 *            The URL to share
	 * @throws FacebookException
	 */
	/*
	 * @Deprecated public void postLink(String link) throws FacebookException { postLink(link, null, null, null, null, null, null,Value.EVERYONE); }
	 */
	 ...
	 */
	/*
	 * @Deprecated public void post(WallPost wallPost) throws FacebookException { post(wallPost, Constants.ME); }
	 */

	/**
	 * Requires the publish_stream permission.
	 * 
	 * To publish a wall post, POST the message and optional attachment to the feed/wall of the user, page or group, i.e., http://graph.facebook.com/PROFILE_ID/feed. When publishing to a Page, to target a post to users in a specific location or language, use the appropriate values from the following
	 * files: all cities (CSV) {@link http://developers.facebook.com/attachment/all_cities_final.csv}, major cities (CSV) {@link http://developers.facebook.com/attachment/major_cities_final.csv}, locales (CSV) {@link http://developers.facebook.com/attachment/locales_final.csv}, cities and locales
	 * {@link http://developers.facebook.com/attachment/targeting_ids.json}
	 * 
	 * @param wallPost
	 * @param profileId
	 * @throws FacebookException
	 */
	/*
	 * @Deprecated public void post(WallPost wallPost, String profileId) throws FacebookException{ List<NameValuePair> namesValues = new ArrayList<NameValuePair>(); Gson gson = new Gson();
	 * 
	 * namesValues.add(new NameValuePair(Constants.PARAM_ACCESS_TOKEN, this.authAccessToken.getAccessToken()));
	 * 
	 * if(wallPost.getActions()!=null){ namesValues.add(new NameValuePair(Constants.ACTIONS,gson.toJson(wallPost.getActions()))); } if(wallPost.getCaption()!=null){ namesValues.add(new NameValuePair(Constants.CAPTION,wallPost.getCaption())); } if(wallPost.getDescription()!=null){ namesValues.add(new
	 * NameValuePair(Constants.DESCRIPTION,wallPost.getDescription())); } if(wallPost.getLink()!=null ){ namesValues.add(new NameValuePair(Constants.LINK,wallPost.getLink())); } if(wallPost.getMessage()!=null){ namesValues.add(new NameValuePair(Constants.MESSAGE, wallPost.getMessage())); }
	 * if(wallPost.getName()!=null){ namesValues.add(new NameValuePair(Constants.NAME,wallPost.getName())); } if(wallPost.getPicture()!=null){ namesValues.add(new NameValuePair(Constants.PICTURE,wallPost.getPicture())); } if(wallPost.getPrivacy()!=null){ namesValues.add(new
	 * NameValuePair(Constants.PRIVACY,gson.toJson(wallPost.getPrivacy()))); } if(wallPost.getSource()!=null){ namesValues.add(new NameValuePair(Constants.SOURCE,wallPost.getSource())); } if(wallPost.getTargeting()!=null){ namesValues.add(new
	 * NameValuePair(Constants.TARGETING,gson.toJson(wallPost.getPrivacy()))); }
	 * 
	 * NameValuePair[] nameValuePairs = new NameValuePair[namesValues.size()]; namesValues.toArray(nameValuePairs); caller.postData(Constants.FACEBOOK_GRAPH_URL + Constants.POST_LINK.replaceFirst("PROFILE_ID", profileId), nameValuePairs); }
	 */
	 ...
	 * @param source
	 * @param profileId
	 *            The user id of the user on whose wall the message needs to be posted, if null then the post would be posted on the authenticated users wall. If it's a page then page id.
	 * @throws FacebookException
	 */
	public String wallPost(String message, String picture, String link, String name, String caption, String description, String source, String profileId) throws FacebookException {
		...
		return caller.postData(Constants.FACEBOOK_GRAPH_URL + Constants.POST_FEED.replaceFirst(Constants.REPLACE_PROFILE_ID, profileId), nameValuePairs);
	 ...
	 * @param endTime
	 * @param profileId
	 *            If for a user then the users id, if for a page then the page id
	 * @throws FacebookException
	 */
	...
	// TODO: Upload photo
	/*
	 * public void photos(String name, String message, String profileId) throws FacebookException { List<NameValuePair> namesValues = new ArrayList<NameValuePair>();
	 * 
	 * namesValues.add(new NameValuePair(Constants.PARAM_ACCESS_TOKEN, this.authAccessToken.getAccessToken()));
	 * 
	 * if(profileId == null){ profileId = Constants.ME; }
	 * 
	 * if(name != null){ namesValues.add(new NameValuePair(Constants.NAME,name)); }
	 * 
	 * if(message != null){ namesValues.add(new NameValuePair(Constants.MESSAGE, message)); }
	 * 
	 * NameValuePair[] nameValuePairs = new NameValuePair[namesValues.size()]; namesValues.toArray(nameValuePairs); caller.postData(Constants.FACEBOOK_GRAPH_URL + Constants.POST_EVENTS.replaceFirst(Constants.REPLACE_PROFILE_ID, profileId), nameValuePairs); }
	 */

	// TODO: Publish Checkins
	/*
	 * public void checkins(String coordinates, String place, String message, String tags) throws FacebookException { List<NameValuePair> namesValues = new ArrayList<NameValuePair>();
	 * 
	 * namesValues.add(new NameValuePair(Constants.PARAM_ACCESS_TOKEN, this.authAccessToken.getAccessToken()));
	 * 
	 * if(profileId == null){ profileId = Constants.ME; }
	 ...
	 * if(message != null){ namesValues.add(new NameValuePair(Constants.MESSAGE, message)); }
	 * 
	 * NameValuePair[] nameValuePairs = new NameValuePair[namesValues.size()]; namesValues.toArray(nameValuePairs); caller.postData(Constants.FACEBOOK_GRAPH_URL + Constants.POST_CHECKINS.replaceFirst(Constants.REPLACE_PROFILE_ID, profileId), nameValuePairs); }
	 */
	...
	public FqlPost[] newsFeed(List<StreamColumn> columnNames, StreamColumnCriteria columnCriteria) throws FacebookException {
		...
		String fqlQuery = "SELECT " + columnName.toString() + " FROM stream WHERE filter_key in (SELECT filter_key FROM stream_filter WHERE uid=me() AND type='newsfeed') ";
	...
	public FqlUser[] fqlUsers(List<FqlUserColumn> columnNames, FqlUserColumnCriteria columnCriteria) throws FacebookException {
		...
		String fqlQuery = "SELECT " + columnName.toString() + " FROM user WHERE " + columnCriteria.toString();
	...
	public FqlPage[] fqlPages(List<FqlPageColumn> columnNames, FqlPageColumnCriteria columnCriteria) throws FacebookException {
		...
		String fqlQuery = "SELECT " + columnName.toString() + " FROM page WHERE " + columnCriteria.toString();
	 ...
	 * if (columnCriteria != null) {
	 * 
	 * if (columnCriteria.isDefaultXid()) { criteria.append(" AND " + StreamColumn.XID.toString() + " = 'default'"); } else if (columnCriteria.getXid() != null) { criteria.append(" AND " + StreamColumn.XID.toString() + " = " + columnCriteria.getXid()); }
	 * 
	 * if (columnCriteria.getActorId() != null) { criteria.append(" AND " + StreamColumn.ACTOR_ID.toString() + " = '" + columnCriteria.getActorId() + "'"); }
	 * 
	 * if (columnCriteria.getAppId() != null) { criteria.append(" AND " + StreamColumn.APP_ID.toString() + " = " + columnCriteria.getAppId()); }
	 * 
	 * if (columnCriteria.getAttribution() != null) { criteria.append(" AND " + StreamColumn.ATTRIBUTION.toString() + " = '" + columnCriteria.getAttribution() + "'"); }
	 * 
	 * if (columnCriteria.getCreatedTimeGreaterThan() != null) { criteria.append(" AND " + StreamColumn.CREATED_TIME.toString() + " > " + columnCriteria.getCreatedTimeGreaterThan()); }
	 * 
	 * if (columnCriteria.getCreatedTimeLessThan() != null) { criteria.append(" AND " + StreamColumn.CREATED_TIME.toString() + " < " + columnCriteria.getCreatedTimeLessThan()); }
	 * 
	 * if (columnCriteria.getFilterKey() != null) { criteria.append(" AND " + StreamColumn.FILTER_KEY.toString() + " = '" + columnCriteria.getFilterKey() + "'"); }
	 * 
	 * if (columnCriteria.getPostId() != null) { criteria.append(" AND " + StreamColumn.POST_ID.toString() + " = '" + columnCriteria.getPostId() + "'"); }
	 * 
	 * if (columnCriteria.getSourceId() != null) { criteria.append(" AND " + StreamColumn.SOURCE_ID.toString() + " = " + columnCriteria.getSourceId()); }
	 * 
	 * if (columnCriteria.getTargetId() != null) { criteria.append(" AND " + StreamColumn.TARGET_ID.toString() + " = '" + columnCriteria.getTargetId() + "'"); }
	 * 
	 * if (columnCriteria.getUpdatedTimeGreaterThan() != null) { criteria.append(" AND " + StreamColumn.UPDATED_TIME.toString() + " > " + columnCriteria.getUpdatedTimeGreaterThan()); }
	 * 
	 * if (columnCriteria.getUpdatedTimeLessThan() != null) { criteria.append(" AND " + StreamColumn.UPDATED_TIME.toString() + " < " + columnCriteria.getUpdatedTimeLessThan()); }
	 * 
	 * if (columnCriteria.getViewerId() != null) { criteria.append(" AND " + StreamColumn.VIEWER_ID.toString() + " = " + columnCriteria.getViewerId()); }
	 * 
	 * // We will always be passing this param // TODO: Do we need to always set this? if (columnCriteria.isShowHidden()) { criteria.append(" AND " + isHidden + " = 0 "); } else { // criteria.append(" AND " + isHidden + " = 1 "); }
	 * 
	 * // This should be in the end if (columnCriteria.getLimit() != null) { criteria.append(" LIMIT " + columnCriteria.getLimit()); }
	 * 
	 * }
	 * 
	 * return criteria; }
	 ...
	 * <b>Post</b> = An individual entry in a profile's feed. The read_stream extended permission is required to access any information in a profile's feed that is not shared with everyone.
	 * 
	 * @param postId
	 * @return
	 * @throws FacebookException
	...
	/**
	 * Returns the appropriate object for the given connection type. Facebook objects, apart from their fields have connections such as likes, comments etc. This method will return the connections corresponding to the type that you specify
	 * 
	 * @param <E>
	 * @param id
	 *            The facebook object id
	 * @param connectionType
	 *            Enum, send in what the type of the connection is
	 * @param e
	...
	/**
	 * This has been done so that we touch the paging object inside the connection. Paging object contains private fields which need to be modfied. The private fields get modified on calling any of the getters. This is needed because gson directly calls private fields. It does not call the getters.
	 * Since the private fields depend on the getters, we are touching the getter so that private fields get initialized accordingly.
	 * 
	 * @param <E>
	 * @param e
	 */
	 ...
	 * @param url
	 * @param e
	 *            The class into which the json object returned by the url fetch needs to be cast
	 * @param nameValuePairs
	 *            Pass parameters that need to accompany the call
	 * @return
	 * @throws FacebookException
	 */
