...
public final class RenrenApiInvoker {
	...
	public RenrenApiInvoker(String token, boolean isAccessToken) {
		this.token = token;
	...
	public String uploadPhoto(TreeMap<String, String> parameters, String filename, String contentType, byte[] data) {
		prepareParams(parameters, FORMAT_JSON);
		String content = HttpURLUtils.doUploadFile(RenrenApiConfig.renrenApiUrl, parameters, "upload", filename, contentType, data);
		if (content.indexOf("error_code") >= 0) {
			throw this.parseRenrenApiException(content);
		}
		return content;
